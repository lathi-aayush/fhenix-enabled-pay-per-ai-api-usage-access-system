...
export default function Drafts() {
  ...
  return (
    <div className="pt-6 max-w-4xl">
      ...
      {!isAuthenticated && <GuestConnectBanner message="Connect MetaMask to view your drafts." className="mb-6" />}
          ...
          <li key={p._id} className="bg-white border border-surface-variant rounded-md px-4 py-3 flex flex-wrap justify-between gap-2">
            <div>
              ...
              <div className="text-xs text-on-surface-variant">
                {p.projectId?.title || "Project"} Â· updated {p.updatedAt ? new Date(p.updatedAt).toLocaleString() : ""}
              </div>
            </div>
