    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-on-surface-variant mt-0.5 truncate">
          {post.projectId?.title ? `${post.projectId.title} Â· ` : ""}
...
export default function StudioHome() {
  ...
  const greetingLine = useMemo(() => {
    if (drafts.length > 0) {
      return `${timeGreeting()}, ${firstName} â€” you have ${drafts.length} draft${drafts.length === 1 ? "" : "s"} ready to publish.`;
    }
    if (lastWorkflow?.name) {
      return `Welcome back, ${firstName} â€” continue working on ${lastWorkflow.name}.`;
    }
    if (lastProject?.title) {
      return `Welcome back, ${firstName} â€” your last session: ${lastProject.title}.`;
    }
    return `${timeGreeting()}, ${firstName} â€” your workspace is ready.`;
  ...
  return (
    <div className="pt-4 pb-8">
      {!isAuthenticated && (
        <GuestConnectBanner
          message="Browse Studio tools freely. Connect MetaMask to save projects, generate content, and publish."
          className="mb-6"
        />
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px] pb-8 border-b border-slate-200">
        <div>
          <h1 className="font-headline text-2xl font-semibold text-primary leading-snug">
            {isAuthenticated ? greetingLine : `${timeGreeting()} â€” explore AI Studio tools for creators.`}
          </h1>
        ...
        <div className="space-y-4">
          <div className="bg-slate-50/80 border border-slate-200 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
              <h2 className="text-sm font-semibold text-primary">ðŸ“… Scheduled This Week</h2>
      ...
      {/* Workflow templates â€” visible to guests */}
      <section className="py-8 border-b border-slate-200">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="font-semibold text-primary">Workflow templates</h2>
            <p className="text-xs text-slate-500 mt-0.5">Pre-built AI pipelines â€” connect wallet to use one</p>
          </div>
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-slate-500">
                      {stats.drafts} draft{stats.drafts === 1 ? "" : "s"} Â· {stats.published} published
                    </p>
                  </div>
                </Link>
              </motion.div>
          ...
          {projects.length > 0 && (
            <div className="min-w-[180px] shrink-0 bg-white border border-surface-variant rounded-lg p-4 flex flex-col justify-center">
              ...
              <p className="text-xs text-slate-500 mt-2">
                {totalDrafts} drafts Â· {totalPublished} published
              </p>
            </div>
          )}
        </div>
        {projects.length === 0 && (
          <p className="text-sm text-on-surface-variant mt-2">No projects yet â€” create one to organize your blogs.</p>
        )}
      </section>

      {/* Recent drafts â€” 3 on home; rest on All drafts */}
